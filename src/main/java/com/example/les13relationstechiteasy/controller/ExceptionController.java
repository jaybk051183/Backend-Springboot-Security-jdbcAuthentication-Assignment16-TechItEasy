...
import com.example.les13relationstechiteasy.exceptions.BadRequestException;
import com.example.les13relationstechiteasy.exceptions.RecordNotFoundException;
import com.example.les13relationstechiteasy.exceptions.TelevisionNameTooLongException;
import com.example.les13relationstechiteasy.exceptions.UsernameNotFoundException;
...
public class ExceptionController {
    ...
    //Deze exception handler vangt elke BadRequestException op die naar de gebruiker gegooid wordt en returned daar voor in de plaats een ResponseEntity met de Message en de NOT_FOUND-status (404)
    @ExceptionHandler(value = BadRequestException.class)
    public ResponseEntity<Object> exception(BadRequestException exception) {
        return new ResponseEntity<>(exception.getMessage(), HttpStatus.BAD_REQUEST);
    }

    //Deze exception handler vangt elke UsernameNotFoundException op die naar de gebruiker gegooid wordt en returned daar voor in de plaats een ResponseEntity met de Message en de NOT_FOUND-status (404)
